...
openai_assistant_prompt = "You specialize in text stemming, text stemming is a natural language processing technique that is used to reduce words to their base form, also known as the root form. The process of stemming is used to normalize text and make it easier to process. You should stem every word except from names of people, places, organisations, brands. For example, the words “programming,” “programmer,” and “programs” can all be reduced down to the common stem “program”. However, in the sentence “Programs PTY. LTD. sold for 1 billion euros”, the word “programs” should not be stemmed because it refers to the name of a company.\n" \

openai_prompt = openai_assistant_prompt + f"Can you provide the stemmed version of this paragraph?\n{paragraph_sample}\nStemmed paragraph:\n{stemmed_sample}\n" \
                                          "Can you provide the stemmed version of this paragraph?\n{paragraph}\nStemmed paragraph:"


llama_prompt = f"{BOS}{B_INST} {B_SYS}\n" \
                "You specialize in text stemming, text stemming is a natural language processing technique that is used to reduce words to their base form, also known as the root form. The process of stemming is used to normalize text and make it easier to process. You should stem every word except from names of people, places, organisations, brands. For example, the words “programming,” “programmer,” and “programs” can all be reduced down to the common stem “program”. However, in the sentence “Programs PTY. LTD. sold for 1 billion euros”, the word “programs” should not be stemmed because it refers to the name of a company.\n" \
                f"{E_SYS}\n\n" \
                f"Can you provide the stemmed version of this paragraph?\n{paragraph_sample} " \
                f"{E_INST} Stemmed paragraph:\n{stemmed_sample} {EOS}{BOS}{B_INST} " \
                "Can you provide the stemmed version of this paragraph?\n{paragraph}\n " \
                f"{E_INST} Stemmed paragraph:\n" \
...
def get_openai_response(prompt, out_token_count, api_key):
...
def get_llama_batch_responses(prompts, model, tokenizer, tem_ids):
    ...
    #if shape is larger than max_length, then increase and encode again
    if encoded_inputs["input_ids"].shape[1]>=1280:
        encoded_inputs = tokenizer.batch_encode_plus(
            prompts,
            truncation=True,  # Truncate to model's max length
            padding='longest',
    ...
    for i, response in enumerate(response_sentences):
        try:
            if E_INST in response:
                ...
                current_response = response.split("Stemmed paragraph:")[2]
...
def worker_openai(api_key, model, paragraph):
    encoding = tiktoken.encoding_for_model("gpt-3.5-turbo")
    prompt = openai_prompt.format(paragraph=paragraph["contents"])
    token_count = len(encoding.encode(prompt))

    response = get_openai_response(prompt, token_count, api_key)
    if response is None:
        return None

    response_dict = {"id": paragraph["id"], "contents": response}
...
def main():
    ...
    model = arg.model
    out_folder = f"output_{model}_{input_file.split('/')[0]}_context"
        ...
        if model == "openai":
            with open(output_file, "a+") as fw:
                with Pool(processes=3) as pool:
                    func = partial(worker_openai, api_key, model)
        ...
        for chunk in tqdm(chunked_collection_list):
            ...
            with open(output_file, "a+") as f:
                for id, response in zip(new_ids, responses):
                    response_dict = {"id": id, "contents": response}
